...
version: 
Author: vansson
Date: 2023-12-29 09:57:36
LastEditors: vansson
LastEditTime: 2024-01-11 16:50:07
'''
# from PIL import Image
# import colorsys
# import os

# def calculate_sv_values(image_path):
...
# folder_path = '/media/vansson/etc/downloads/qqfile/data_hsv/data_hsv/H_ORANGE'

# for filename in os.listdir(folder_path):
#     if filename.endswith('.jpg') or filename.endswith('.png'):
#         image_path = os.path.join(folder_path, filename)
#         s_values, v_values = calculate_sv_values(image_path)
#         avg_s = sum(s_values) / len(s_values)
#         avg_v = sum(v_values) / len(v_values)
        
#         print(f"图片:{filename}, s值:{avg_s},v值:{avg_v}")
...
# 原图片、标签文件、裁剪图片路径
img_path = r'/media/vansson/etc/downloads/qqfile/data_hsv/data_hsv/H_YELLOW'
...
def three_line(map1, map2, map3):
    ...
    plt.plot(x, y3, '^-', color="#D190F0", label="H-HSV-H")
    for a, b in zip(x, y3):
        plt.text(a, b + 0.05, '%.0f' % b, ha='center', va='bottom', fontsize=8)
    plt.xlabel("H VALUES OF ORANGE SAFETY HELMET")
    plt.ylabel("FREQUENCY")
